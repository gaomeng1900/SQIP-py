...
# alter table old_application add column term int(11) DEFAULT 0;
...
class Application(db.Model):
	...
	def __init__(self, in_charge_id, pro_name="", tutors_expected="", tutor_id="", submit_or=False, review_state=0, term=0):
...
class OldApplication(db.Model):
	...
	def __init__(self, args):
		self.in_charge_id    = args.in_charge_id
		self.pro_name        = args.pro_name
		self.tutors_expected = args.tutors_expected
		self.tutor_id        = args.tutor_id
		self.submit_or       = args.submit_or
		self.review_state    = args.review_state
		self.term 			 = args.term

	def __repr__(self):
		return '<OldApplication %r %s %s %s %s %s %s> ' % (self.id, self.pro_name, self.in_charge_id, self.tutors_expected, self.tutor_id, self.submit_or, self.review_state)
...
def submitApplication(yb_id, form):
	...
	thisApplication.term    		= meta.getTerm()
...
def getAllOld():
	allApls = OldApplication.query.all()
	db.session.close()
	return allApls
	...
	if apls:
		for apl in apls:
			if apl.submit_or is True:
				...
				apl_info = {"id":apl.id, 
							"pro_name":apl.pro_name , 
							"in_charge_stu_name":in_charge_stu_name, 
							"tutors_name_expected":tutors_name_expected,
							"in_charge_id":apl.in_charge_id, 
							"review_state":apl.review_state}	
...
def changeState(id, state):
	...
	if not thisApplication:
		...
		return 2
	state = int(state)
	thisApplication.review_state = state
	db.session.add(thisApplication)
	db.session.commit()
	db.session.close()
	return 0
...
def archive():
	apls = getAllOldApplications()
	for apl in apls:
		if apl.submit_or is True:
			yb_id = apl.in_charge_id

			db.session.add(OldApplication(apl))
			db.session.commit()
			db.session.close()
